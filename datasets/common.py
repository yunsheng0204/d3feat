...
class Dataset:
    ...
    def tf_augment_input(self, stacked_points, batch_inds, config):
        ...
        # Parameter: How many point clouds are in this batch.
        ...
        ##########
        if config.augment_rotation == 'axes':
            # Choose a random angle and a random axis for each element
            ...
            if axis == 0:
                ...
                R = tf.stack([c, cs0, s, cs0, cs1, cs0, -s, cs0, c], axis=1)
            else:
                R = tf.stack([cs1, cs0, cs0, cs0, c, -s, cs0, s, c], axis=1)
            R = tf.reshape(R, (-1, 3, 3))

            # Create N * 3 * 3 rotation matrices to multiply with stacked_points
        ...
        elif config.augment_rotation == 'vertical':
    ...
    def init_input_pipeline(self, config):
        ...
        print("self.batch_limit:", self.batch_limit)
        ...
        gen_function_val, _, _ = self.get_batch_gen('val', config)
        map_func = self.get_tf_mapping(config)
    ...
    def init_test_input_pipeline(self, config):
        ...
        # TODO: repeat, or the last test ply cannot be used.
    ...
    def tf_descriptor_input(self,
        ...
        min_len = tf.reduce_min(stacked_lengths, keep_dims=True)
        batch_weights = tf.cast(min_len, tf.float32) / tf.cast(stacked_lengths, tf.float32)
        stacked_weights = tf.gather(batch_weights, batch_inds)
        # starting radius
        r_normal = config.first_subsampling_dl * config.KP_extent * 2.5
        ...
        for block_i, block in enumerate(config.architecture):
            ...
            if layer_blocks:
                ...
                if np.any(['deformable' in blck for blck in layer_blocks[:-1]]):
                    ...
                    r = r_normal
                conv_i = tf_batch_neighbors(stacked_points, stacked_points, stacked_lengths, stacked_lengths, r)
            else:
                # This layer only perform pooling, no neighbors required
                conv_i = tf.zeros((0, 1), dtype=tf.int32)
            ...
            if 'pool' in block or 'strided' in block:
                ...
                # Subsampled points
                pool_p, pool_b = tf_batch_subsampling(stacked_points, stacked_lengths, sampleDl=dl)
                ...
                # Subsample indices
                pool_i = tf_batch_neighbors(pool_p, stacked_points, pool_b, stacked_lengths, r)

                # Upsample indices (with the radius of the next layer to keep wanted density)
                up_i = tf_batch_neighbors(stacked_points, pool_p, stacked_lengths, pool_b, 2 * r)
