...
class OxfordKITTIDataset(Dataset):
    AUGMENT = None
    DATA_FILES = {
    ...
    def __init__(self, input_threads=8, first_subsampling_dl=0.30, load_test=False):
        Dataset.__init__(self, 'Oxford')
        self.network_model = 'descriptor'
        self.num_threads = input_threads
        self.load_test = load_test
        self.root = 'data/oxford'
    ...
    def __getitem__(self, split, idx):
        ...
        src_name, tgt_name, tf_name = self.files[split][idx]

        src_path = os.path.join(self.root, 'bin', src_name.replace('.ply', '.bin'))
        tgt_path = os.path.join(self.root, 'bin', tgt_name.replace('.ply', '.bin'))

        tf_path = os.path.join(self.root, 'transforms', tf_name)

        xyzr0 = np.fromfile(src_path, dtype=np.float32).reshape(-1, 4)
        xyzr1 = np.fromfile(tgt_path, dtype=np.float32).reshape(-1, 4)

        xyz0 = xyzr0[:, :3]
        xyz1 = xyzr1[:, :3]

        trans = np.loadtxt(tf_path).astype(np.float32)
        ...
        return (
            anc_points,
            pos_points,
            unaligned_anc_points,
            unaligned_pos_points,
            matches,
            trans,
            True
        )
