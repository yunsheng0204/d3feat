...
def position_aware_local_encoding_lite(features, points, neighbors, out_dim=None, geo_dim=16, name='pare_lite_position_encoding'):
    """
    Memory-safe PARE-style geometric descriptor refinement.

    This avoids dense layers on [N, K, C] tensors.
    It only uses compact local geometry statistics [N, 5].
    """
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        neighbor_points = tf.gather(points_with_shadow, neighbors)
        center_points = tf.expand_dims(points, axis=1)

        relative_xyz = neighbor_points - center_points
        relative_dist = tf.norm(relative_xyz + 1e-9, axis=-1, keepdims=True)

        mean_xyz = tf.reduce_mean(relative_xyz, axis=1)
        mean_dist = tf.reduce_mean(relative_dist, axis=1)
        max_dist = tf.reduce_max(relative_dist, axis=1)

        local_geo = tf.concat([mean_xyz, mean_dist, max_dist], axis=-1)
        ...
        refined = tf.layers.dense(tf.concat([features, local_geo], axis=-1),
                                  out_dim,
                                  activation=None,
                                  name='refine_fc')
...
def assemble_FCNN_blocks(inputs, config, dropout_prob):
    """
    D3Feat + PARE-Lite descriptor branch.
    Detection score branch remains original D3Feat.
    """
    ...
    # PARE-Lite descriptor refinement
    backup_features = position_aware_local_encoding_lite(
        backup_features,
        inputs['points'][0],
        inputs['neighbors'][0],
        out_dim=backup_features.get_shape()[-1].value,
        geo_dim=16,
        name='kitti_pare_lite_descriptor_refinement'
    )

    # Original D3Feat soft detection module
