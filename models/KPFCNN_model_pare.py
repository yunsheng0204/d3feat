...
from models.D3Feat_oxford_pare import assemble_FCNN_blocks
    ...
    def __init__(self, flat_inputs, config):
        ...
        if self.config.saving:
            if self.config.saving_path == None:
                ...
                if os.path.exists(os.path.join('.', 'training_oxford_pare.py')):
                    shutil.copy2(os.path.join('.', 'training_oxford_pare.py'), os.path.join(snapshot_root, 'train.py'))
                shutil.copy2(os.path.join('.', 'utils/trainer.py'), os.path.join(snapshot_root, 'trainer.py'))
                shutil.copy2(os.path.join('.', 'models/D3Feat_oxford_pare.py'), os.path.join(snapshot_root, 'model.py'))
